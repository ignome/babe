                ...
                if (url == "") {
                  ...
                  function (data) {
                    if(data.code == 100){
                      ...
                      if(data.product.pid){
                        $("#J_ShareGoodsD").hide();
                        $.guang.ugc.goodsExist(data.product,data.userSns,getBaobei);
                      }else{
                        $("#J_ShareGoodsD").hide();
                        $.guang.ugc.goodspub(data.product,false,data.userSns,getBaobei);
                      }
                    ...
                    if (data.code == 100) {
                      $(".text-tip").html('');
                      $.guang.ugc.goodspub(data.product, data.isUploadRole, data.userSns, getBaobei);
...
<OBJECT style="VISIBILITY: visible" classid=clsid:D27CDB6E-AE6D-11cf-96B8-444553540000 width=' + _width + ' height=' + _height + '>\
  ...
  <img alt="获取flash播放器" src="http://www.adobe.com/images/shared/download_buttons/get_flash_player.gif"/>\
    ...
    decodeContent4Sns : function (html) {
      ...
      html = $.guang.util.ellipse(html,120);
      return html;
    }
  }
